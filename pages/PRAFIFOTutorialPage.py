"""
PRA FIFO Tutorial Page - Step-by-step walkthrough of FIFO Page Replacement Algorithm
Progression is column-by-column with visual queue updates
"""
...
class PRAFIFOTutorialPage(QWidget):
    """Step-by-step FIFO Page Replacement Algorithm tutorial"""
    ...
    def _load_step_templates(self):
        ...
        possible_paths = [
            os.path.join(os.path.dirname(__file__), '..', 'tutorial_kb', 'pra_fifo_steps.json'),
            'tutorial_kb/pra_fifo_steps.json',
        ]
    ...
    def _generate_random_problem(self):
        """Generate random frame data and page sequence (same logic as PRA main window)"""
    ...
    def _on_table_cell_changed(self, row, column):
        ...
        if self._updating_table:
            return
        if column == 0:  # Don't allow Frame ID changes
            return
    ...
    def _generate_steps(self):
        ...
        step_num = 2
        solution_grid = {}  # {(frame_id, col_index): {"text": page, "is_hit": bool}}
        ...
        for col_idx, requested_page in enumerate(self.page_sequence):
            ...
            if hit_frame:
                ...
                # Update queue: move victim to back
                current_queue.pop(0)
    ...
    def setup_ui(self):
        ...
        # Title
        self.page_title = QLabel("FIFO Page Replacement Tutorial")
        self.page_title.setObjectName("tutorialTitle")
        layout.addWidget(self.page_title)
        
        # Frame input section (editable)
        ...
        # Frame table (editable - Frame ID, Load Time, Page in Memory)
        ...
        # Solution table (Frame ID, Page in Memory, then page sequence columns)
    ...
    def _setup_solution_table(self):
        ...
        # Columns: Frame ID, Page in Memory, then one for each page request
    ...
    def _update_queue_display(self, queue_order):
        ...
        # Add arrow labels between blocks
        # Note: We rebuild from scratch each time, so we don't need to remove arrows
    ...
    def _update_solution_table(self, solution_grid, frame_state):
        ...
        # Update "Page in Memory" column with current frame state
        for frame_id, page in frame_state.items():
        ...
        for (frame_id, col_idx), cell_data in solution_grid.items():
            row = frame_to_row.get(frame_id)
            if row is not None:
                col = col_idx + 2  # Offset for Frame ID and Page in Memory columns
                item = self.solution_table.item(row, col)
                if item:
