"""
SJF Priority Tutorial Page - Step-by-step walkthrough of SJF with Priority (Preemptive)
Uses ORIGINAL burst for comparison, priority preempts, same-priority shorter-burst preempts
"""
...
class SJFPriorityTutorialPage(QWidget):
    """Step-by-step SJF Priority tutorial with preemptive logic"""
    ...
    def _load_step_templates(self):
        possible_paths = [
            os.path.join(os.path.dirname(__file__), '..', 'tutorial_kb', 'sjf_priority_steps.json'),
            'tutorial_kb/sjf_priority_steps.json',
        ]
    ...
    def _generate_steps(self):
        """Generate steps for SJF Priority (preemptive, uses ORIGINAL burst)"""
        self.steps = []
        templates = self.step_templates.get("step_types", {})
        
        # Track process state - store ORIGINAL burst separately
        process_map = {p['id']: p.copy() for p in self.processes}
        for pid in process_map:
            process_map[pid]['remaining'] = process_map[pid]['burst']
            process_map[pid]['original_burst'] = process_map[pid]['burst']  # Never changes
        ...
        while current_time <= 32:
                ...
                for new_proc in new_arrivals:
                    ...
                    if new_proc['priority'] > current_process['priority']:
                        ...
                        desc = tmpl.get("description", "").format(
                            ...
                            current_time=current_time, old_remaining=current_process['remaining'],
                            old_burst=current_process['original_burst']
                        )
                    # Same priority, shorter ORIGINAL burst
                    elif new_proc['priority'] == current_process['priority'] and new_proc['original_burst'] < current_process['original_burst']:
                        should_preempt = True
                        tmpl = templates.get("preemption_burst", {})
                        title = tmpl.get("title", "PREEMPTION!").format(step=step_num, new_process=new_proc['id'], old_process=current_process['id'])
                        desc = tmpl.get("description", "").format(
                            new_process=new_proc['id'], old_process=current_process['id'],
                            priority=new_proc['priority'], new_burst=new_proc['original_burst'],
                            old_burst=current_process['original_burst'], current_time=current_time,
            ...
            if current_process is None and ready_queue:
                # Sort by: priority (desc), original_burst (asc), arrival (asc), id (asc)
            ...
            block_num = current_process['original_burst'] - current_process['remaining'] + 1
            remaining_after = current_process['remaining'] - 1
            is_last = (remaining_after == 0)
            
            ready_list = ", ".join([f"{p['id']}(P{p['priority']},B{p['original_burst']})" for p in sorted(ready_queue, key=lambda x: (-x['priority'], x['original_burst']))])
            
            if block_num == 1:
                tmpl = templates.get("first_block", {})
                title = tmpl.get("title", "").format(step=step_num, process_id=pid, priority=current_process['priority'], burst=current_process['original_burst'])
                desc = tmpl.get("description", "").format(
                    step=step_num, process_id=pid, priority=current_process['priority'],
                    burst=current_process['original_burst'], current_time=current_time, ready_list=ready_list or "none"
                )
            elif is_last:
                end_time = current_time + 1
                end_times[pid] = end_time
                tat = end_time - current_process['arrival']
                wait = tat - current_process['original_burst']
                tmpl = templates.get("process_complete", {})
                title = tmpl.get("title", "").format(step=step_num, process_id=pid)
                desc = tmpl.get("description", "").format(
                    step=step_num, process_id=pid, priority=current_process['priority'],
                    burst=current_process['original_burst'], start_time=start_times.get(pid, current_time),
                ...
                desc = tmpl.get("description", "").format(
                    step=step_num, process_id=pid, block_num=block_num,
                    burst=current_process['original_burst'], remaining=remaining_after, current_time=current_time
                )
    ...
    def setup_ui(self):
        ...
        self.page_title = QLabel("SJF Priority Tutorial")
