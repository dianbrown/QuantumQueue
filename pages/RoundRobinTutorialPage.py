"""
Round Robin Tutorial Page - Step-by-step walkthrough of Round Robin scheduling
Quantum-based time slice with editable quantum field
"""
...
class RoundRobinTutorialPage(QWidget):
    """Step-by-step Round Robin tutorial with editable quantum"""
    ...
    def __init__(self, parent=None):
        ...
        self.processes = []
        self.steps = []
        self.quantum = 2  # Default quantum
    ...
    def _load_step_templates(self):
        possible_paths = [
            os.path.join(os.path.dirname(__file__), '..', 'tutorial_kb', 'round_robin_steps.json'),
            'tutorial_kb/round_robin_steps.json',
        ]
    ...
    def _generate_steps(self):
        """Generate steps for Round Robin (quantum-based)"""
        self.steps = []
        ...
        arrival_info = "\n".join([f"- {p['id']}: RS = {p['arrival']} (arrival)" for p in sorted(self.processes, key=lambda x: x['arrival'])])
        ...
        while current_time <= 32:
            ...
            if current_process and quantum_used >= quantum and current_process['remaining'] > 0:
                ...
                desc = tmpl.get("description", "").format(
                    process_id=current_process['id'], quantum=quantum,
            ...
            if current_process is None and ready_queue:
                # Sort by rs_time (asc), then by arrival (desc - newest first)
                ready_queue.sort(key=lambda p: (p['rs_time'], -p['arrival']))
            ...
            ready_list = ", ".join([f"{p['id']}(RS={p['rs_time']})" for p in sorted(ready_queue, key=lambda x: (x['rs_time'], -x['arrival']))])
            
            if is_first_in_quantum and not is_last:
                run_time = min(quantum, current_process['remaining'])
                tmpl = templates.get("first_block", {})
                title = tmpl.get("title", "").format(step=step_num, process_id=pid, quantum_used=quantum_used, quantum=quantum)
                desc = tmpl.get("description", "").format(
                    current_time=current_time, ready_list=ready_list or "empty",
                    process_id=pid, rs_time=current_process['rs_time'],
                    quantum=quantum, remaining=current_process['remaining'], run_time=run_time
                )
            elif is_last:
                ...
                desc = tmpl.get("description", "").format(
                    process_id=pid, quantum_used=quantum_used, quantum=quantum,
                    burst=current_process['burst'], arrival=current_process['arrival'],
                    end_time=end_time, wait_time=max(0, wait), tat=tat
                )
            else:
                ...
                desc = tmpl.get("description", "").format(
                    process_id=pid, quantum_used=quantum_used, quantum=quantum,
                    remaining=remaining_after, current_time=current_time
                )
    ...
    def setup_ui(self):
        ...
        self.page_title = QLabel("Round Robin Tutorial")
