"""
Round Robin Priority Tutorial Page - Step-by-step walkthrough
Priority preemption + quantum-based time slice with editable quantum field
"""
...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, 
                              ...
                              QScrollArea, QHeaderView, QSpinBox)
...
class RoundRobinPriorityTutorialPage(QWidget):
    ...
    def _generate_random_processes(self):
        ...
        self.quantum = ProcessGenerator.generate_quantum()
        if hasattr(self, 'quantum_spinbox'):
            self._updating_table = True
            self.quantum_spinbox.setValue(self.quantum)
            self._updating_table = False
    ...
    def _generate_steps(self):
        """Generate steps for Round Robin Priority (priority preemption + quantum)"""
        self.steps = []
        ...
        process_list = ", ".join([f"{p['id']}(P{p['priority']},B{p['burst']})" for p in self.processes])
        arrival_info = "\n".join([f"- {p['id']}: P={p['priority']}, RS={p['arrival']}" for p in sorted(self.processes, key=lambda x: x['arrival'])])
        ...
        self.steps.append({
            ...
            "description": initial.get("description", "").format(
                num_processes=len(self.processes), process_list=process_list, quantum=quantum
            ),
            "timeline": {}, "rs_markers": {}
        })
        ...
        self.steps.append({
            ...
            "description": arrivals.get("description", "").format(arrival_info=arrival_info),
            "timeline": {}, "rs_markers": {k: v.copy() for k, v in rs_markers.items()}
        })
        
        # Simulate Round Robin Priority
        step_num = 2
        current_time = 1
        timeline = {}
        ...
        while current_time <= 32:
            ...
            # Check for PRIORITY preemption (higher priority preempts mid-quantum)
            if new_arrivals and current_process:
                for new_proc in new_arrivals:
                    if new_proc['priority'] > current_process['priority']:
                        ...
                        process_map[current_process['id']]['rs_time'] = current_time
                        ...
                        tmpl = templates.get("preemption_priority", {})
                        ...
                        desc = tmpl.get("description", "").format(
                            new_process=new_proc['id'], new_priority=new_proc['priority'],
                            old_process=current_process['id'], old_priority=current_process['priority'],
                            current_time=current_time, quantum_used=quantum_used, quantum=quantum,
                            old_rs=current_time  # Updated RS time
                        )
                        self.steps.append({
            ...
            # Check quantum expiration (only if no priority preemption)
            if current_process and quantum_used >= quantum and current_process['remaining'] > 0:
                current_process['rs_time'] = current_time
                process_map[current_process['id']]['rs_time'] = current_time
                ...
                tmpl = templates.get("quantum_expire", {})
                title = tmpl.get("title", "Quantum expires!").format(step=step_num, process_id=current_process['id'])
                desc = tmpl.get("description", "").format(
                    process_id=current_process['id'], quantum=quantum, priority=current_process['priority'],
                    remaining=current_process['remaining'], new_rs=current_time
                )
                self.steps.append({
                    "title": title, "description": desc,
                    "timeline": {k: v.copy() for k, v in timeline.items()},
            ...
            if current_process is None and ready_queue:
                # Sort by: priority (desc), rs_time (asc), arrival (desc - newest first)
                ready_queue.sort(key=lambda p: (-p['priority'], p['rs_time'], -p['arrival']))
            ...
            remaining_after = current_process['remaining'] - 1
            is_first_in_quantum = (quantum_used == 1)
            is_last = (remaining_after == 0)
            
            ready_list = ", ".join([f"{p['id']}(P{p['priority']},RS={p['rs_time']})" for p in sorted(ready_queue, key=lambda x: (-x['priority'], x['rs_time']))])
            
            if is_first_in_quantum and not is_last:
                tmpl = templates.get("first_block", {})
                title = tmpl.get("title", "").format(step=step_num, process_id=pid, priority=current_process['priority'], quantum_used=quantum_used, quantum=quantum)
                desc = tmpl.get("description", "").format(
                    current_time=current_time, ready_list=ready_list or "empty",
                    process_id=pid, priority=current_process['priority'], rs_time=current_process['rs_time']
                )
            elif is_last:
                ...
                desc = tmpl.get("description", "").format(
                    process_id=pid, priority=current_process['priority'], burst=current_process['burst'],
                    end_time=end_time, quantum_used=quantum_used, quantum=quantum,
                    wait_time=max(0, wait), tat=tat
                )
            else:
                tmpl = templates.get("continue_quantum", {})
                title = tmpl.get("title", "").format(step=step_num, process_id=pid, priority=current_process['priority'], quantum_used=quantum_used, quantum=quantum)
                desc = tmpl.get("description", "").format(
                    process_id=pid, priority=current_process['priority'],
                    quantum_used=quantum_used, quantum=quantum, remaining=remaining_after
                )
            ...
            current_process['remaining'] = remaining_after
            process_map[pid]['remaining'] = remaining_after
        ...
        for proc in self.processes:
            ...
            total_tat += tat
            metrics_lines.append(f"- {pid}: P={proc['priority']}, Burst={proc['burst']}, WT={max(0,wait)}, TAT={tat}")
        
        final = templates.get("final", {})
        self.steps.append({
            "title": final.get("title", "Final").format(step=step_num),
            "description": final.get("description", "").format(
                quantum=quantum, execution_order=exec_order_str, metrics="\n".join(metrics_lines),
    ...
    def setup_ui(self):
        ...
        self.page_title = QLabel("Round Robin Priority Tutorial")
        ...
        title_layout.addWidget(quantum_label)
        
        self.quantum_spinbox = QSpinBox()
        self.quantum_spinbox.setRange(1, 10)
        self.quantum_spinbox.setValue(self.quantum)
        self.quantum_spinbox.setObjectName("quantumSpinbox")
        self.quantum_spinbox.valueChanged.connect(self._on_quantum_changed)
        title_layout.addWidget(self.quantum_spinbox)
    ...
    def apply_theme(self, theme: dict):
        ...
        self.setStyleSheet(f"""
            ...
            QLabel#quantumLabel {{ font-size: 18px; font-weight: bold; color: {text_color}; background-color: transparent; }}
            QSpinBox#quantumSpinbox {{ background-color: {input_bg}; border: 1px solid {table_grid}; padding: 8px; border-radius: 4px; color: {text_color}; font-size: 16px; min-width: 60px; }}
