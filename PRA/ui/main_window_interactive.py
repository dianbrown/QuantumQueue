...
class PRAMainWindow(QWidget):
    ...
    def setup_frame_table(self):
        ...
        self.frame_table.setHorizontalHeaderLabels(["Frame ID", "Load Time", "Pages in Memory"])
    ...
    def add_sample_frames(self):
        ...
        sample_frames = [
            Frame("0", 11, "7,0,1,2,0,3,0,4"),
            Frame("1", 8, "7,0,1,2,0,3,0,4"),
            Frame("2", 1, "7,0,1,2,0,3,0,4"),
        ]
    ...
    def fill_solution_table_structure(self):
        ...
        for frame_idx, frame in enumerate(self.frames):
            ...
            load_item.setFlags(load_item.flags() & ~Qt.ItemIsEditable)
    ...
    def compare_solutions(self, student_solution):
        """Compare student solution with correct solution."""
        ...
        for access_idx, access in enumerate(self.solution_result.accesses):
            if access_idx < len(student_solution):
                student_access = student_solution[access_idx]
                
                # Check each frame
                for frame_idx in range(len(self.frames)):
                    total_checks += 1
                    expected_page = access.frames_state[frame_idx]
                    
                    if frame_idx in student_access:
                        student_page = student_access[frame_idx]['page']
                        student_is_hit = student_access[frame_idx]['is_hit']
                        
                        # Check page content
                        if student_page == expected_page:
                            correct_count += 1
                        else:
                            errors.append(f"Time {access_idx}: Frame {frame_idx} should be '{expected_page}', got '{student_page}'")
                            
                        # Check hit/fault coloring for accessed page
                        if student_page == access.page_id:
                            expected_is_hit = access.is_hit
                            if student_is_hit != expected_is_hit:
                                color_type = "hit" if expected_is_hit else "fault"
                                errors.append(f"Time {access_idx}: Page {access.page_id} should be marked as {color_type}")
                    else:
                        if expected_page is not None:
                            errors.append(f"Time {access_idx}: Frame {frame_idx} should contain '{expected_page}' but is empty")
                        else:
                            correct_count += 1
